...
let inventory = JSON.parse(localStorage.getItem("printteesInventory")) || {};
...
document.getElementById("addForm").addEventListener("submit", function (e) {
  ...
  localStorage.setItem("printteesInventory", JSON.stringify(inventory));

  saveSuggestion("printteesStyleSuggestions", styleInput);
  saveSuggestion("printteesColors_" + style, colorInput);
  renderInventory();
  e.target.reset();
});
...
loadSuggestions("printteesStyleSuggestions", "styleSuggestions");

document.getElementById("style").addEventListener("change", function () {
  const style = this.value.trim().toLowerCase();
  loadSuggestions("printteesColors_" + style, "colorSuggestions");
});
